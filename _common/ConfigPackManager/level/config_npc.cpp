...
bool NpcSetup::parse(QSettings *setup, QString npcImgPath, int defaultGrid, NpcSetup *merge_with, QString *error)
{
    ...
    defGFX_h = gfx_h / int(frames*uint(pow(2, framestyle)));
    ...
    z_offset =             setup->value("z-offset", merge_with ? merge_with->z_offset : 0.0f).toDouble();
    ...
    /***************GRID And snap*********************************/
    grid = setup->value("grid", merge_with ? merge_with->grid : defaultGrid).toInt();
        NumberLimiter::apply(grid, 1);
    ...
    if( width >= uint(grid) )
        grid_offset_x = -1 * qRound( qreal(int(width) % grid)/2 );
    else
        grid_offset_x = qRound( qreal( grid - int(width)) / 2.0 );

    if(grid_attach_style==1)
        grid_offset_x += (grid/2);

    grid_offset_y = -int(height) % grid;
    /***************Calculate the grid offset********************/
    ...
    iTmp =      setup->value("default-friendly", -1).toInt();
        ...
        default_friendly_value = (iTmp>=0) ? bool(iTmp) : (merge_with ? merge_with->default_friendly_value : false);

    iTmp =      setup->value("default-no-movable", -1).toInt();
        default_nomovable = (iTmp>=0);
        default_nomovable_value = (iTmp>=0)?bool(iTmp) : (merge_with ? merge_with->default_nomovable_value : false);

    iTmp =      setup->value("default-is-boss", -1).toInt();
        default_boss = (iTmp>=0);
        default_boss_value = (iTmp>=0) ? bool(iTmp) : (merge_with ? merge_with->default_boss_value : false);
...
void NpcSetup::applyNPCtxt(const NPCConfigFile *local, const NpcSetup global, const QSize captured)
{
    ...
    framespeed = (local->en_framespeed)? qRound( qreal(global.framespeed) / (qreal(8) / qreal(local->framespeed)) ) : global.framespeed;
    framestyle = (local->en_framestyle)?local->framestyle:global.framestyle;
    ...
    if(((int)width>=(int)grid))
        grid_offset_x = -1 * qRound( qreal((int)width % grid)/2 );
    else
        grid_offset_x = qRound( qreal( grid - (int)width )/2 );


    grid_attach_style = (local->en_grid_align)?local->grid_align:global.grid_attach_style;

    if(grid_attach_style==1) grid_offset_x += (grid/2);

    grid_offset_y = -height % grid;
    ...
    {
        frames = qRound(qreal(captured.height())/qreal(gfx_h));
    ...
    if( (framestyle>0) && (gfx_h*frames >= (unsigned int)captured.height()) )
    {
        framestyle = 0;
        ani_direct = false;
    ...
    score = (local->en_score)?local->score:global.score;
    health = (local->en_health)?local->health:global.health;
