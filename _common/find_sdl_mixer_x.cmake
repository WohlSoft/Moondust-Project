...
function(find_mixer_x_real)
    ...
    if(_mixerx_STATIC)
        ...
        mixerx_FindLibrary(${_mixerx_SEARCH_PATHS} xmp FOUND_LIBS)
    ...
    if(NOT SDL2_USE_SYSTEM)
        ...
        list(APPEND FOUND_LIBS ${SDL2_LIBRARIES})
    endif()
...
function(find_mixer_x)
    ...
    if(_mixerx_STATIC)
        if(WIN32)
            ...
            set(SDL_MixerX_A_Lib "${_mixerx_SEARCH_PATHS}/libSDL2_mixer_ext${MIX_DEBUG_SUFFIX}.a")
            list(APPEND FOUND_LIBS "${SDL_MixerX_A_Lib}")
            # Workaround for Ninja
            if(NOT EXISTS "${SDL_MixerX_A_Lib}")
                file(WRITE "${SDL_MixerX_A_Lib}" "dummy")
            endif()
        endif()
    else()
        if(WIN32)
    ...
    if(_mixerx_STATIC)
        ...
        list(APPEND FOUND_LIBS "xmp${MIX_DEBUG_SUFFIX}")
    ...
    if(ANDROID)
        list(APPEND FOUND_LIBS "${_mixerx_SEARCH_PATHS}/libSDL2.so")
        # Workaround for Ninja
        if(NOT EXISTS "${_mixerx_SEARCH_PATHS}/libSDL2.so")
            file(WRITE "${_mixerx_SEARCH_PATHS}/libSDL2.so" "dummy")
        endif()
        if(NOT EXISTS "${_mixerx_SEARCH_PATHS}/libhidapi.so")
            file(WRITE "${_mixerx_SEARCH_PATHS}/libhidapi.so" "dummy")
        endif()
    elseif(NOT SDL2_USE_SYSTEM)
        if(_mixerx_STATIC)
            if(WIN32)
                list(APPEND FOUND_LIBS "${_mixerx_SEARCH_PATHS}/libSDL2-static${MIX_DEBUG_SUFFIX}.a")
            else()
                set(LibSDL_A "${_mixerx_SEARCH_PATHS}/libSDL2${MIX_DEBUG_SUFFIX}.a")
                list(APPEND FOUND_LIBS "${LibSDL_A}")
                # Workaround for Ninja
                if(NOT EXISTS "${LibSDL_A}")
                    file(WRITE "${LibSDL_A}" "dummy")
                endif()
            endif()
        else()
            if(WIN32)
        ...
        if((WIN32 OR HAIKU OR ANDROID) AND NOT EMSCRIPTEN)
