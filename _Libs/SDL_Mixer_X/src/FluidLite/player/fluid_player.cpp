...
#include "fluid_player_private.hpp"
...
ADLMIDI_EXPORT struct Fluid_MIDIPlayer *fluid_play_init(long sample_rate)
{
    Fluid_MIDIPlayer *_device;
    _device = (Fluid_MIDIPlayer *)malloc(sizeof(Fluid_MIDIPlayer));
...
ADLMIDI_EXPORT void fluid_setLoopEnabled(Fluid_MIDIPlayer *device, int loopEn)
{
    if(!device) return;
...
ADLMIDI_EXPORT int fluid_openFile(Fluid_MIDIPlayer *device, char *filePath)
{
...
ADLMIDI_EXPORT int fluid_openData(Fluid_MIDIPlayer *device, void *mem, long size)
{
    ADLMIDI_ErrorString.clear();
...
ADLMIDI_EXPORT const char *fluid_errorString()
{
    return ADLMIDI_ErrorString.c_str();
}

ADLMIDI_EXPORT const char *adl_getMusicTitle(Fluid_MIDIPlayer *device)
{
    if(!device) return "";
...
ADLMIDI_EXPORT void fluid_close(Fluid_MIDIPlayer *device)
{
...
ADLMIDI_EXPORT void fluid_reset(Fluid_MIDIPlayer *device)
{
    if(!device)
        return;
...
inline static void SendStereoAudio(Fluid_MIDIPlayer *device,
...
ADLMIDI_EXPORT int fluid_play(Fluid_MIDIPlayer *device, int sampleCount, short *out)
{
    if(!device)
        return 0;
