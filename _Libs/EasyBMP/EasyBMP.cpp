...
static bool EasyBMPwarnings = true;
...
void BMIH::display(void)
{
    ...
    cout << "biSize: " << biSize << endl
         << "biWidth: " << biWidth << endl
         << "biHeight: " << biHeight << endl
         << "biPlanes: " << biPlanes << endl
         << "biBitCount: " << biBitCount << endl
         << "biCompression: " <<  biCompression << endl
         << "biSizeImage: " << biSizeImage << endl
         << "biXPelsPerMeter: " << biXPelsPerMeter << endl
         << "biYPelsPerMeter: " <<  biYPelsPerMeter << endl
         << "biClrUsed: " << biClrUsed << endl
         << "biClrImportant: " << biClrImportant << endl << endl;
}
...
void BMFH::display(void)
{
    ...
    cout << "bfType: " << bfType << endl
         << "bfSize: " << bfSize << endl
         << "bfReserved1: " << bfReserved1 << endl
         << "bfReserved2: " << bfReserved2 << endl
         << "bfOffBits: " << bfOffBits << endl << endl;
}
...
bool BMP::WriteToFile(const char *FileName)
{
    ...
    fwrite(reinterpret_cast<char *>(&(bmfh.bfType)) , sizeof(ebmpWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmfh.bfSize)) , sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmfh.bfReserved1)) , sizeof(ebmpWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmfh.bfReserved2)) , sizeof(ebmpWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmfh.bfOffBits)) , sizeof(ebmpDWORD) , 1 , fp);
    // write the info header
    BMIH bmih;
    bmih.biSize = 40;
    bmih.biWidth = static_cast<ebmpDWORD>(Width);
    bmih.biHeight = static_cast<ebmpDWORD>(Height);
    bmih.biPlanes = 1;
    bmih.biBitCount = static_cast<ebmpWORD>(BitDepth);
    bmih.biCompression = 0;
    bmih.biSizeImage = static_cast<ebmpDWORD>(dTotalPixelBytes);

    if(XPelsPerMeter)
        bmih.biXPelsPerMeter = static_cast<ebmpDWORD>(XPelsPerMeter);
    else
        bmih.biXPelsPerMeter = DefaultXPelsPerMeter;

    if(YPelsPerMeter)
        bmih.biYPelsPerMeter = static_cast<ebmpDWORD>(YPelsPerMeter);
    else
    ...
    fwrite(reinterpret_cast<char *>(&(bmih.biSize)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biWidth)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biHeight)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biPlanes)), sizeof(ebmpWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biBitCount)), sizeof(ebmpWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biCompression)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biSizeImage)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biXPelsPerMeter)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biYPelsPerMeter)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biClrUsed)), sizeof(ebmpDWORD) , 1 , fp);
    fwrite(reinterpret_cast<char *>(&(bmih.biClrImportant)), sizeof(ebmpDWORD) , 1 , fp);
    ...
    if(BitDepth == 1 || BitDepth == 4 || BitDepth == 8)
    {
        ...
        for(n = 0 ; n < NumberOfColors ; n++)
            fwrite(reinterpret_cast<char *>(&(Colors[n])) , 4 , 1 , fp);
    }
    ...
    if(BitDepth != 16)
    {
        ...
        while(j > -1)
        {
            ...
            {
                int BytesWritten =
                    static_cast<int>(fwrite(
                                         reinterpret_cast<char *>(Buffer),
                                         1, static_cast<size_t>(BufferSize),
                                         fp));
    ...
    if(BitDepth == 16)
    {
        ...
        fwrite(reinterpret_cast<char *>(&RedMask) , 2 , 1 , fp);
        fwrite(reinterpret_cast<char *>(&ZeroWORD) , 2 , 1 , fp);

        if(IsBigEndian())
            GreenMask = FlipWORD(GreenMask);

        fwrite(reinterpret_cast<char *>(&GreenMask) , 2 , 1 , fp);
        fwrite(reinterpret_cast<char *>(&ZeroWORD) , 2 , 1 , fp);

        if(IsBigEndian())
            BlueMask = FlipWORD(BlueMask);

        fwrite(reinterpret_cast<char *>(&BlueMask) , 2 , 1 , fp);
        fwrite(reinterpret_cast<char *>(&ZeroWORD) , 2 , 1 , fp);
        ...
        for(j = Height - 1 ; j >= 0 ; j--)
        {
            ...
            while(WriteNumber < DataBytes)
            {
                ...
                TempWORD =
                    static_cast<ebmpWORD>(RedWORD << 11) +
                    static_cast<ebmpWORD>(GreenWORD << 5) +
                    BlueWORD;

                if(IsBigEndian())
                    TempWORD = FlipWORD(TempWORD);

                fwrite(reinterpret_cast<char *>(&TempWORD), 2, 1, fp);
                WriteNumber += 2;
                i++;
            }
            ...
            {
                ebmpBYTE TempBYTE;
                fwrite(reinterpret_cast<char *>(&TempBYTE), 1, 1, fp);
                WriteNumber++;
            }
        }
    }
...
bool BMP::ReadFromFile(const char *FileName)
{
    ...
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmfh.bfType)), sizeof(ebmpWORD), 1, fp);
    bool IsBitmap = false;
    ...
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmfh.bfSize)) , sizeof(ebmpDWORD) , 1, fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmfh.bfReserved1)) , sizeof(ebmpWORD) , 1, fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmfh.bfReserved2)) , sizeof(ebmpWORD) , 1, fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmfh.bfOffBits)) , sizeof(ebmpDWORD) , 1 , fp);
    ...
    BMIH bmih;
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biSize)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biWidth)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biHeight)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biPlanes)) , sizeof(ebmpWORD) , 1, fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biBitCount)) , sizeof(ebmpWORD) , 1, fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biCompression)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biSizeImage)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biXPelsPerMeter)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biYPelsPerMeter)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biClrUsed)) , sizeof(ebmpDWORD) , 1 , fp);
    NotCorrupted &= SafeFread(reinterpret_cast<char *>(&(bmih.biClrImportant)) , sizeof(ebmpDWORD) , 1 , fp);
    ...
    XPelsPerMeter = static_cast<int>(bmih.biXPelsPerMeter);
    YPelsPerMeter = static_cast<int>(bmih.biYPelsPerMeter);
    ...
    if(BitDepth < 16)
    {
        ...
        for(n = 0; n < NumberOfColorsToRead ; n++)
            SafeFread(reinterpret_cast<char *>(&(Colors[n])) , 4 , 1 , fp);
    ...
    if(BytesToSkip > 0 && BitDepth != 16)
    {
        ...
        SafeFread(reinterpret_cast<char *>(TempSkipBYTE), BytesToSkip , 1 , fp);
        delete [] TempSkipBYTE;
    }
    ...
    if(BitDepth == 16)
    {
        ...
        if(bmih.biCompression != 0)
        {
            ...
            //ebmpWORD ZeroWORD;
            SafeFread(reinterpret_cast<char *>(&RedMask) , 2 , 1 , fp);

            if(IsBigEndian())
                RedMask = FlipWORD(RedMask);

            SafeFread(reinterpret_cast<char *>(&TempMaskWORD) , 2, 1, fp);
            SafeFread(reinterpret_cast<char *>(&GreenMask), 2 , 1 , fp);

            if(IsBigEndian())
                GreenMask = FlipWORD(GreenMask);

            SafeFread(reinterpret_cast<char *>(&TempMaskWORD) , 2, 1, fp);
            SafeFread(reinterpret_cast<char *>(&BlueMask), 2 , 1 , fp);
        ...
        for(j = Height - 1 ; j >= 0 ; j--)
        {
            ...
            {
                ebmpWORD TempWORD;
                SafeFread(reinterpret_cast<char *>(&TempWORD), 2 , 1 , fp);
            ...
            {
                ebmpBYTE TempBYTE;
                SafeFread(reinterpret_cast<char *>(&TempBYTE), 1, 1, fp);
                ReadNumber++;
            }
        }
    }
...
bool BMP::CreateStandardColorTable(void)
{
    ...
    if(BitDepth == 1)
    {
        ...
        for(i = 0 ; i < 2 ; i++)
        {
            Colors[i].Red = static_cast<ebmpBYTE>(i * 255);
            Colors[i].Green = static_cast<ebmpBYTE>(i * 255);
            Colors[i].Blue = static_cast<ebmpBYTE>(i * 255);
            Colors[i].Alpha = 0;
        }
    ...
    if(BitDepth == 4)
    {
        ...
        for(ell = 0 ; ell < 2 ; ell++)
        {
            for(k = 0 ; k < 2 ; k++)
            {
                for(j = 0 ; j < 2 ; j++)
                {
                    Colors[i].Red = static_cast<ebmpBYTE>(j * 128);
                    Colors[i].Green = static_cast<ebmpBYTE>(k * 128);
                    Colors[i].Blue = static_cast<ebmpBYTE>(ell * 128);
                    i++;
                }
            }
        }
        ...
        for(ell = 0 ; ell < 2 ; ell++)
        {
            for(k = 0 ; k < 2 ; k++)
            {
                for(j = 0 ; j < 2 ; j++)
                {
                    Colors[i].Red = static_cast<ebmpBYTE>(j * 255);
                    Colors[i].Green = static_cast<ebmpBYTE>(k * 255);
                    Colors[i].Blue = static_cast<ebmpBYTE>(ell * 255);
                    i++;
                }
            }
        }
    ...
    if(BitDepth == 8)
    {
        ...
        for(ell = 0 ; ell < 4 ; ell++)
        {
            for(k = 0 ; k < 8 ; k++)
            {
                for(j = 0; j < 8 ; j++)
                {
                    Colors[i].Red = static_cast<ebmpBYTE>(j * 32);
                    Colors[i].Green = static_cast<ebmpBYTE>(k * 32);
                    Colors[i].Blue = static_cast<ebmpBYTE>(ell * 64);
                    Colors[i].Alpha = 0;
                    i++;
                }
            }
...
bool SafeFread(char *buffer, int size, int number, FILE *fp)
{
    ...
    ItemsRead = static_cast<int>(fread(buffer,
...
int BMP::TellVerticalDPI(void)
{
    ...
    return static_cast<int>(static_cast<double>(YPelsPerMeter) / 39.37007874015748);
}
...
int BMP::TellHorizontalDPI(void)
{
    ...
    return static_cast<int>(static_cast<double>(XPelsPerMeter) / 39.37007874015748);
}
...
BMFH GetBMFH(const char *szFileNameIn)
{
    ...
    SafeFread(reinterpret_cast<char *>(&(bmfh.bfType)) , sizeof(ebmpWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmfh.bfSize)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmfh.bfReserved1)) , sizeof(ebmpWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmfh.bfReserved2)) , sizeof(ebmpWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmfh.bfOffBits)) , sizeof(ebmpDWORD) , 1 , fp);
    fclose(fp);
...
BMIH GetBMIH(const char *szFileNameIn)
{
    ...
    for(i = 14 ; i > 0 ; i--)
        SafeFread(reinterpret_cast<char *>(&TempBYTE), sizeof(ebmpBYTE) , 1, fp);

    // read the bmih
    SafeFread(reinterpret_cast<char *>(&(bmih.biSize)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biWidth)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biHeight)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biPlanes)) , sizeof(ebmpWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biBitCount)) , sizeof(ebmpWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biCompression)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biSizeImage)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biXPelsPerMeter)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biYPelsPerMeter)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biClrUsed)) , sizeof(ebmpDWORD) , 1 , fp);
    SafeFread(reinterpret_cast<char *>(&(bmih.biClrImportant)) , sizeof(ebmpDWORD) , 1 , fp);
    fclose(fp);
...
bool CreateGrayscaleColorTable(BMP &InputImage)
{
    ...
    if(BitDepth != 1)
        StepSize = static_cast<ebmpBYTE>(255 / (NumberOfColors - 1));
    else
        StepSize = 255;

    for(i = 0 ; i < NumberOfColors ; i++)
    {
        ebmpBYTE TempBYTE = static_cast<ebmpBYTE>(i * StepSize);
        RGBApixel TempColor;
...
bool BMP::Read32bitRow(ebmpBYTE *Buffer, int BufferSize, int Row)
{
    ...
    for(i = 0 ; i < Width ; i++)
        memcpy(reinterpret_cast<char *>(&(Pixels[i][Row])),
               reinterpret_cast<char *>(Buffer) + 4 * i, 4);
...
bool BMP::Read24bitRow(ebmpBYTE *Buffer, int BufferSize, int Row)
{
    ...
    for(i = 0 ; i < Width ; i++)
        memcpy(reinterpret_cast<char *>(&(Pixels[i][Row])),
               reinterpret_cast<char *>(Buffer) + 3 * i, 3);
...
bool BMP::Write32bitRow(ebmpBYTE *Buffer, int BufferSize, int Row)
{
    ...
    for(i = 0 ; i < Width ; i++)
        memcpy(reinterpret_cast<char *>(Buffer) + 4 * i,
               reinterpret_cast<char *>(&(Pixels[i][Row])), 4);
...
bool BMP::Write24bitRow(ebmpBYTE *Buffer, int BufferSize, int Row)
{
    ...
    for(i = 0 ; i < Width ; i++)
        memcpy(reinterpret_cast<char *>(Buffer) + 3 * i,
               reinterpret_cast<char *>(&(Pixels[i][Row])), 3);
...
bool Rescale(BMP &InputImage , char mode, int NewDimension)
{
    ...
    for(int j = 0; j < NewHeight - 1 ; j++)
    {
        ...
        for(int i = 0; i < NewWidth - 1 ; i++)
        {
            ...
            ThetaI -= I;
            InputImage(i, j)->Red = static_cast<ebmpBYTE>
            ...
            InputImage(i, j)->Green = static_cast<ebmpBYTE>
            ...
            InputImage(i, j)->Blue = static_cast<ebmpBYTE>
        ...
        InputImage(NewWidth - 1, j)->Red = static_cast<ebmpBYTE>
                                           ((1.0 - ThetaJ) * (OldImage(OldWidth - 1, J)->Red)
                                            + ThetaJ * (OldImage(OldWidth - 1, J + 1)->Red));
        InputImage(NewWidth - 1, j)->Green = static_cast<ebmpBYTE>
                                             ((1.0 - ThetaJ) * (OldImage(OldWidth - 1, J)->Green)
                                              + ThetaJ * (OldImage(OldWidth - 1, J + 1)->Green));
        InputImage(NewWidth - 1, j)->Blue = static_cast<ebmpBYTE>
    ...
    for(int i = 0 ; i < NewWidth - 1 ; i++)
    {
        ...
        I = static_cast<int>(floor(ThetaI));
        ThetaI -= I;
        InputImage(i, NewHeight - 1)->Red = static_cast<ebmpBYTE>
                                            ((1.0 - ThetaI) * (OldImage(I, OldHeight - 1)->Red)
                                             + ThetaI * (OldImage(I, OldHeight - 1)->Red));
        InputImage(i, NewHeight - 1)->Green = static_cast<ebmpBYTE>
                                              ((1.0 - ThetaI) * (OldImage(I, OldHeight - 1)->Green)
                                               + ThetaI * (OldImage(I, OldHeight - 1)->Green));
        InputImage(i, NewHeight - 1)->Blue = static_cast<ebmpBYTE>
