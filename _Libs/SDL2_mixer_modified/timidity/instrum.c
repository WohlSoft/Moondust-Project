        ...
        if(bank->tone[i].layer && bank->tone[i].last_used < how_old)
        {
            if(bank->tone[i].layer != MAGIC_LOAD_INSTRUMENT)
            {
                ctl->cmsg(CMSG_INFO, VERB_DEBUG,
                          (char *)"Unloading %s %s[%d,%d] - last used %d.",
                                        ...
                                        int strip_tail, int bank, int gm_num, int sf_ix)
{
    ...
    if(noluck)
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                  (char *)"Instrument `%s' can't be found.", name);
        return 0;
    }
    ...
    if((239 != fread(tmp, 1, 239, fp)) ||
       (memcmp(tmp, "GF1PATCH110\0ID#000002", 22) &&
        memcmp(tmp, "GF1PATCH100\0ID#000002", 22))) /* don't know what the
                              differences are */
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, (char *)"%s: not an instrument", name);
        return 0;
    }
    ...
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                  (char *)"Can't handle patches with %d instruments", tmp[82]);
        return 0;
    }
    ...
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                  (char*)"Can't handle instruments with %d layers", tmp[151]);
        return 0;
    }
    ...
    for(vlayer = 0; vlayer < vlayer_count; vlayer++)
    {
        ...
        ctl->cmsg(CMSG_INFO, VERB_NOISY, (char*)"%s%s[%d,%d] %s(%d-%d layer %d of %d)",
                  (percussion) ? "   " : "", name,
...
#define READ_LONG(thing) \
    ...
    thing = LE_LONG(tmplong);
                ...
                {
fail:
                    ctl->cmsg(CMSG_ERROR, VERB_NORMAL, (char*)"Error reading sample %d", i);
                    if(stereo_layer == 1)
                    {
                        for(j = 0; j < i; j++)
