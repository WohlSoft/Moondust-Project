...
contains(DEFINES, USE_NATIVE_MIDI): {
    SOURCES += native_midi/native_midi_common.c
    win32: {
        SOURCES += native_midi/native_midi_win32.c
    }
    macx: {
        SOURCES += \
    ...
    SOURCES += native_midi/native_midi_haiku.cpp
    HEADERS += \
        native_midi/native_midi.h \
        native_midi/native_midi_common.h
}

contains(DEFINES, USE_TIMIDITY_MIDI): {
    HEADERS += \
        timidity/common.h \
        timidity/config.h \
        timidity/ctrlmode.h \
...
contains(DEFINES, USE_FLUIDSYNTH_MIDI): {
    HEADERS += \
...
contains(DEFINES, MODPLUG_MUSIC): {
    DEFINES += \
        MODPLUG_STATIC \
        MODPLUG_BUILD=1
...
contains(DEFINES, MP3_MAD_MUSIC): {
    HEADERS += \
        libid3tag/compat.h \
        libid3tag/crc.h \
        libid3tag/debug.h \
