    ...
    if(drumsflag)
    {
        ...
        if(ip->type == INST_GUS && ip->samples != 1)
        {
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE,
                      (char *)"Strange: percussion instrument with %d samples!", ip->samples);
        }
...
{
    int i = voices;
    ctl->cmsg(CMSG_INFO, VERB_DEBUG, (char *)"All notes off on channel %d", c);
    while(i--)
        if(voice[i].status == VOICE_ON &&
           voice[i].channel == c)
        {
        ...
        while(current_event->time <= current_sample)
        {
            ...
            case ME_EOT:
                ...
                ctl->cmsg(CMSG_INFO, VERB_VERBOSE,
                          (char *)"Playing time: ~%d seconds", current_sample / play_mode->rate + 2);
                ctl->cmsg(CMSG_INFO, VERB_VERBOSE,
                          (char *)"Notes cut: %d", cut_notes);
                ctl->cmsg(CMSG_INFO, VERB_VERBOSE,
                          (char *)"Notes lost totally: %d", lost_notes);
                midi_playing = 0;
                return RC_TUNE_END;
            }
            current_event++;
