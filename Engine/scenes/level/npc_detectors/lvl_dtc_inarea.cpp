...
bool InAreaDetector::detected(long type, long ID)
{
    ...
    if(ID == 0)
    {
        ...
        {
        case 1:
            return !detectedBLK.empty();
        case 2:
            return !detectedBGO.empty();
        case 3:
            return !detectedNPC.empty();
        case 4:
            return !detectedPLR.empty();
        default:
            break;
        }
    }
    else
    {
        ...
        {
        case 1:
            return detectedBLK.find(ID) != detectedBLK.end();
        case 2:
            return detectedBGO.find(ID) != detectedBGO.end();
        case 3:
            return detectedNPC.find(ID) != detectedNPC.end();
        case 4:
            return detectedPLR.find(ID) != detectedPLR.end();
        default:
            break;
        }
    }
...
luabind::adl::object InAreaDetector::getBlocks(lua_State *L)
{
    ...
    int i = 1;
    for(std::vector<LVL_Block *>::iterator it = detectedBlocks.begin(); it != detectedBlocks.end(); it++)
    {
...
luabind::adl::object InAreaDetector::getBGOs(lua_State *L)
{
    ...
    int i = 1;
    for(std::vector<LVL_Bgo *>::iterator it = detectedBGOs.begin(); it != detectedBGOs.end(); it++)
    {
...
luabind::adl::object InAreaDetector::getNPCs(lua_State *L)
{
    ...
    int i = 1;
    for(std::vector<LVL_Npc *>::iterator it = detectedNPCs.begin(); it != detectedNPCs.end(); it++)
    {
        if((*it)->type != PGE_Phys_Object::LVLNPC)
            continue;
...
luabind::adl::object InAreaDetector::getPlayers(lua_State *L)
{
    ...
    int i = 1;
    for(std::vector<LVL_Player *>::iterator it = detectedPlayers.begin(); it != detectedPlayers.end(); it++)
    {
