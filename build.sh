...
do
    case "$var" in
        ...
        qt-png)
            ...
            ;;
        system-libs)
            CMAKE_EXTRA_ARGS="${CMAKE_EXTRA_ARGS} -DUSE_SYSTEM_LIBC=ON -DUSE_SYSTEM_LIBFREETYPE=ON -DUSE_SYSTEM_LIBPNG=ON -DUSE_SYSTEM_LIBJPEG=ON -DUSE_SYSTEM_SDL2=ON -DPGE_STATIC_SDLMIXER=OFF -DUSE_SHARED_FREEIMAGE=ON"
            # -DUSE_SYSTEM_SQLITE=ON # No longer used for a while!
            ;;
        noqt)
...
if $USE_QT6; then
    CMAKE_EXTRA_ARGS="${CMAKE_EXTRA_ARGS} -DPGE_FORCE_QT5=OFF"
fi
